    ...
    for ill,ll in enumerate(lines):
        if ill > 0:
            ...
            if (tmp[0] == basin_id):
                ...
                climate_index = {}
                climate_index['aridity']        = np.float(tmp[1].strip())
                climate_index['seasonality']    = np.float(tmp[2].strip())
                climate_index['precip_as_snow'] = np.float(tmp[3].strip())
...
plt.title(str2tex("Climate gradients from Knoben indices",usetex=usetex))
...
for ibasin_id,basin_id in enumerate(basin_ids):
    ...
    # color based on Knoben climate index
    icolor = tuple( [climate_indexes[basin_id]['red'], climate_indexes[basin_id]['green'], climate_indexes[basin_id]['blue']] )
...
# cbar.set_label(str2tex("Climate gradient from Knoben indexes",usetex=usetex))
