...
sobol_indexes = np.array(sobol_indexes)

min_sobol_indexes = np.min(sobol_indexes)
max_sobol_indexes = np.max(sobol_indexes)
...
for iprocess, process in enumerate(processes):
    ...
    if iprocess > 2:                     # <<<<<<<<<<<<<<<<<<<< use this to start plots from with later processes; if -1, does all processes
        ...
        plt.title(str2tex(processes[iprocess],usetex=usetex))
        ...
        for ibasin_id,basin_id in enumerate(basin_ids):
            ...
            # color based on sensitivity index
            #print('sobol_indexes[ibasin_id,iprocess] = ',sobol_indexes[ibasin_id,iprocess])
            #print('min_sobol_indexes = ',min_sobol_indexes)
            #print('max_sobol_indexes = ',max_sobol_indexes)
            #print('icolor = ',np.int((sobol_indexes[ibasin_id,iprocess]-min_sobol_indexes)/(max_sobol_indexes-min_sobol_indexes)*(len(cc)-1)))
            icolor = cc[np.int((sobol_indexes[ibasin_id,iprocess]-min_sobol_indexes)/(max_sobol_indexes-min_sobol_indexes)*(len(cc)-1))]
            

            # # color based on basin #
        ...
        # -------------------------------------------------------------------------
        iplot += 1
        sub    = fig.add_axes(position(1,1,1,hspace=hspace,vspace=vspace, left=0.2, right=0.45, top=0.778, bottom=0.772) )   # top=0.642, bottom=0.632

        cbar = mpl.colorbar.ColorbarBase(sub, norm=mpl.colors.Normalize(vmin=min_sobol_indexes, vmax=max_sobol_indexes), cmap=cmap, orientation='horizontal')
        cbar.set_label(str2tex("Total Sobol' index $\overline{ST}_i^w$",usetex=usetex))
